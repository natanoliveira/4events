...
define("DADO_EXISTENTE", "Existe um pessoa com o mesmo nome");
...
function show()
{
    ...
    if ($resultado) {
        ...
        die('Erro na listagem de pessoas: ' . $conexao->error);
    }
...
function getById($id)
{
    ...
    if ($resultado) {
        ...
        die('Erro ao obter detalhes da pessoa: ' . $conexao->error);
    }
...
function insert($nome, $sobrenome)
{
    ...
    $query = "INSERT INTO $TabelaBase (nome, sobrenome, token) VALUES (?, ?, UUID())";
    $stmt = $conexao->prepare($query);
    ...
    if ($stmt->execute()) {
        ...
        die('Erro ao adicionar nova pessoa: ' . $conexao->error);
    }

    return ["messag" => FALHA];
}

function edit($id, $nome, $sobrenome)
{
    global $conexao, $TabelaBase;

    $query = "UPDATE $TabelaBase SET nome = ?, sobrenome = ? WHERE id = ?";
    $stmt = $conexao->prepare($query);
    ...
    if ($stmt->execute()) {
        ...
        die('Erro ao atualizar a pessoa: ' . $conexao->error);
    }
...
function delete($id)
{
    ...
    // vamos verificar se o pessoa ainda existe em banco de dados para retornar a mensagem ao front
    $deletado = getById($id);
    ...
    if ($stmt->execute()) {
        ...
        die('Erro ao excluir a pessoa: ' . $conexao->error);
    }
...
function editSituation($id, $situacao)
{
    ...
    if ($stmt->execute()) {
        ...
        die('Erro ao atualizar pessoa: ' . $conexao->error);
    }
