...
switch ($method) {
    ...
    case 'POST':
        $input = json_decode(file_get_contents('php://input'), true);
        if ($request[0] == 'new' && isset($input['nome'], $input['descricao'])) {
            echo json_encode(insert($input['nome'], $input['descricao']));
        } else {
            http_response_code(400);
            echo json_encode(['error' => 'Parâmetros inválidos']);
        }
    ...
    case 'PUT':
        $input = json_decode(file_get_contents('php://input'), true);
        if ($request[0] == 'update' && isset($request[1], $input['nome'], $input['descricao'])) {
            echo json_encode(edit($request[1], $input['nome'], $input['descricao']));
        }
